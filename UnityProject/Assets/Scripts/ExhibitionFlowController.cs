...
namespace ShadowPrototype
{
    ...
    {
        private const string DefaultCaptureCameraArguments = "--mode live --camera 0 --no-camera-fallback --width 640 --height 480 --fps 15";
        private const string DefaultHandTrackingCameraArguments = "--camera 1 --skip-camera 0 --width 320 --height 240 --fps 10 --retry-forever --retry-interval 3 --packet-width 640 --packet-height 480 --udp-host 127.0.0.1 --udp-port 5053";

        [Header("Flow")]
        [SerializeField] private bool autoStartOnPlay = true;
        [SerializeField] private bool suppressExistingMeshOnPlay = true;
        [SerializeField] private bool autoStartHandTrackingOnPlay = true;
        ...
        [SerializeField] private bool useExternalBackendOnWindows;
        ...
        [SerializeField] private bool showSf3dButton;
        [SerializeField] private string sf3dButtonLabel = "Save PNG + SF3D";
        ...
        [SerializeField] private string capturePythonRelativePath = ".venv/Scripts/python.exe";
        ...
        [SerializeField] private string handTrackingPythonRelativePath = ".venv/Scripts/python.exe";
            ...
            Sf3dPngPipelineClient sf3dClient = null)
        {
            ...
            {
                captureWorkingDirectory = GetProjectRoot();
            }

            if (string.IsNullOrWhiteSpace(handTrackingWorkingDirectory))
            {
                handTrackingWorkingDirectory = GetProjectRoot();
            }
        ...
        private void ApplyDefaultCameraArguments()
        {
            ...
            if (string.IsNullOrWhiteSpace(trimmedCaptureArguments) ||
                ...
                !trimmedCaptureArguments.Contains("--width"))
            {
            ...
            if (string.IsNullOrWhiteSpace(trimmedHandTrackingArguments) ||
                ...
                trimmedHandTrackingArguments.Contains("--no-camera-fallback") ||
                trimmedHandTrackingArguments.Contains("--width 640 --height 480 --fps 15") ||
                !trimmedHandTrackingArguments.Contains("--width") ||
                !trimmedHandTrackingArguments.Contains("--packet-width"))
            {
        ...
        private void DrawSf3dButton()
        {
            ...
            Rect buttonRect = new Rect(18.0f, 18.0f, buttonWidth, buttonHeight);
            bool isSf3dRunning = sf3dPngPipelineClient != null && sf3dPngPipelineClient.IsRunning;
            bool previousEnabled = GUI.enabled;
            GUI.enabled = previousEnabled && !isSf3dRunning;

            string label = isSf3dRunning ? "SF3D Running..." : sf3dButtonLabel;
        ...
        public void StartExhibitionFlow()
        {
            ...
            if (autoStartHandTrackingOnPlay)
            {
        ...
        private void RequestShadowSilhouetteExport()
        {
            ...
            if (autoStartSf3dAfterPngExport &&
                sf3dPngPipelineClient != null &&
                sf3dPngPipelineClient.IsRunning)
            {
                ShowExportStatus("SF3D is already running.");
                return;
            }
        ...
        private void ExportCurrentShadowSilhouette()
        {
            ...
            if (autoStartSf3dAfterPngExport && sf3dPngPipelineClient != null)
            {
                Debug.Log("Shadow silhouette export requested for in-memory SF3D pipeline.");
                if (shadowDeformer.TryEncodeSilhouetteToPng(
                        out byte[] pngBytes,
                        exportResolution,
                        exportFillColor == default ? Color.black : exportFillColor,
                        exportBackgroundColor))
                {
                    ShowExportStatus("Sending shadow silhouette to ControlNet + SF3D.");
                    sf3dPngPipelineClient.GenerateFromPngBytes(pngBytes, exportFileName);
                    gameManager?.OnShadowMeshExtracted();
                    return;
                }

                ShowExportStatus("PNG encode failed. Check Unity Console.");
                return;
            }
            ...
            string scriptArguments)
        {
            string command =
                ...
                $"& {QuotePowerShellArgument(pythonPath)} {QuotePowerShellArgument(scriptPath)}";
        ...
        private static string ResolvePythonPath(string workingDirectory, string preferredRelativePath)
        {
            ...
            foreach (string environmentVariableName in new[] { "CAP2_PYTHON", "PYTHON" })
            {
                string configuredPythonPath = Environment.GetEnvironmentVariable(environmentVariableName);
            ...
            foreach (string executableName in new[] { "python.exe", "python3.exe", "python" })
            {
                if (TryFindExecutableOnPath(executableName, out string executablePath))
                {
        ...
        private static bool TryFindExecutableOnPath(string executableName, out string executablePath)
        {
            ...
            foreach (string directory in pathValue.Split(Path.PathSeparator))
            {
                ...
                try
                {
                    candidatePath = Path.Combine(directory.Trim(), executableName);
                }
