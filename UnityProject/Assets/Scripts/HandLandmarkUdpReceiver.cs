...
namespace ShadowPrototype
{
    public class HandLandmarkUdpReceiver : MonoBehaviour
    {
        ...
        public void StartReceiver()
        {
            ...
            Debug.Log($"HandLandmarkUdpReceiver listening on UDP port {port}.");
        ...
        private void ReceiveLoop()
        {
            while (isRunning)
            {
                try
                {
                    ...
                    long receivedCount = Interlocked.Increment(ref packetCount);
                    if (!loggedFirstPacket)
                    {
                        loggedFirstPacket = true;
                        pendingInfo = $"HandLandmarkUdpReceiver received first packet on UDP port {port}: {parsedLandmarks.Length} landmarks.";
                    }

                    if (printPacketsToConsole)
                    {
                        pendingInfo = $"HandLandmarkUdpReceiver packet #{receivedCount}: {latestPacket}";
                    }
                }
                catch (SocketException)
                {
