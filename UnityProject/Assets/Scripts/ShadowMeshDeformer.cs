...
namespace ShadowPrototype
{
    ...
    public class ShadowMeshDeformer : MonoBehaviour
    {
        private const float TransientRelaxationSpeed = 6.0f;
        private const float ColliderRefreshInterval = 0.08f;
        ...
        public void ReplaceMesh(Mesh newMesh, int[] newBoundaryIndices = null)
        {
            if (newMesh == null)
            {
                Debug.LogWarning("ShadowMeshDeformer: received a null mesh; keeping the current mesh.");
                return;
            }
        ...
        public bool TryEncodeSilhouetteToPng(out byte[] pngBytes, int resolution = 1024, Color? fillColor = null, Color? bgColor = null)
        {
            pngBytes = Array.Empty<byte>();
            if (!HasMesh)
            {
                Debug.LogWarning("ShadowMeshDeformer: no mesh is loaded for export.");
                return false;
            }
            ...
            {
                Debug.LogWarning($"ShadowMeshDeformer: silhouette PNG encode failed: {exception.Message}");
                return false;
            }
            finally
            {
