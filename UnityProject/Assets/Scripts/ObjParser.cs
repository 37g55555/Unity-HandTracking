...
namespace ShadowPrototype
{
    public static class ObjParser
    {
        public static Mesh Parse(string path)
        {
            ...
            {
                Debug.LogWarning("ObjParser: file was not found.");
                return null;
            }
            ...
            try
            {
                foreach (string rawLine in File.ReadAllLines(path))
                {
                    ...
                    if (line.StartsWith("f ", StringComparison.Ordinal))
                    {
                        ...
                        {
                            Debug.LogWarning("ObjParser: only triangle faces are supported.");
                            continue;
                        }
            ...
            {
                Debug.LogWarning($"ObjParser: file read failed for '{path}': {exception.Message}");
                return null;
            }
            catch (UnauthorizedAccessException exception)
            {
                Debug.LogWarning($"ObjParser: file access failed for '{path}': {exception.Message}");
                return null;
            }

            if (vertices.Count == 0 || triangles.Count == 0)
            {
                Debug.LogWarning($"ObjParser: no valid mesh data found in '{path}'.");
                return null;
            }
