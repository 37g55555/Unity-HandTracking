...
namespace ShadowPrototype
{
    ...
    public class ShadowDeformer : MonoBehaviour
    {
        ...
        public bool SaveSilhouetteToPng(string outputPath, int resolution = 1024, Color? fillColor = null, Color? bgColor = null)
        {
            if (!HasMesh)
            {
                Debug.LogWarning("ShadowDeformer: 저장할 메시가 없음");
                return false;
            }

            Color fill = fillColor ?? Color.black;
            Color bg   = bgColor   ?? Color.clear;

            // 픽셀 버퍼 초기화
            var pixels = new Color[resolution * resolution];
            for (int i = 0; i < pixels.Length; i++) pixels[i] = bg;

            // 버텍스를 UV 공간(0~1)으로 정규화
            Bounds bounds = meshFilter.sharedMesh.bounds;
            float rangeX = bounds.size.x > 0.0001f ? bounds.size.x : 1f;
            float rangeY = bounds.size.y > 0.0001f ? bounds.size.y : 1f;

            int[] triangles = GetActiveTriangles();
            for (int i = 0; i < triangles.Length; i += 3)
            {
                Vector2 a = NormalizeVertex(workingVertices[triangles[i]],     bounds, rangeX, rangeY, resolution);
                Vector2 b = NormalizeVertex(workingVertices[triangles[i + 1]], bounds, rangeX, rangeY, resolution);
                Vector2 c = NormalizeVertex(workingVertices[triangles[i + 2]], bounds, rangeX, rangeY, resolution);

                // 삼각형 AABB로 범위 제한 후 픽셀 채우기
                int minX = Mathf.Max(0, Mathf.FloorToInt(Mathf.Min(a.x, b.x, c.x)));
                int maxX = Mathf.Min(resolution - 1, Mathf.CeilToInt(Mathf.Max(a.x, b.x, c.x)));
                int minY = Mathf.Max(0, Mathf.FloorToInt(Mathf.Min(a.y, b.y, c.y)));
                int maxY = Mathf.Min(resolution - 1, Mathf.CeilToInt(Mathf.Max(a.y, b.y, c.y)));
            ...
            // 텍스처 생성 및 저장
            var texture2D = new Texture2D(resolution, resolution, TextureFormat.ARGB32, false);
            ...
            try
            {
                ...
                Debug.Log($"ShadowDeformer: 실루엣 저장 완료 → {outputPath}");
                success = true;
            }
            catch (Exception e)
            {
                Debug.LogWarning($"ShadowDeformer: 실루엣 PNG 저장 실패 → {e.Message}");
            }
            finally
            {
        ...
        // 버텍스를 픽셀 좌표로 변환
        private static Vector2 NormalizeVertex(Vector3 v, Bounds bounds, float rangeX, float rangeY, int resolution)
        {
            float u = (v.x - bounds.min.x) / rangeX;
            float w = (v.y - bounds.min.y) / rangeY;
            return new Vector2(u * (resolution - 1), w * (resolution - 1));
        }
    }
}
